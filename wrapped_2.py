...
def associated_tasks(award_list,data,spec,kb,kb2):
    ...
    # return tweets_dict, full_nom_dict, final_nom, final_pres, final_winners, compressed_dict
...
# Load in answers - we will remove this when we submit the final final
...
# Associated People Test


# final_nom, final_winner,final_pres = associated_tasks(people_awards, data, "people", people, people);
# tweets_dict, full_nom_dict, final_nom, final_pres, final_winners, compress = associated_tasks(media_awards, data, "media", media, people);

# Pulled functions for presenters testing
# award_list = people_awards
# cat_filter_dict = get_awards_dict(award_list)                     
# tweets_dict = get_all_awards_tweets(award_list, cat_filter_dict, data)
# full_nom_dict = get_nominees(award_list, cat_filter_dict, tweets_dict, "people")
# final_winners = get_people_winners(full_nom_dict)
...
# people_compressed = associated_tasks(people_awards, data, "people", people, people)
#full_people_dict = associated_tasks(people_awards, data, "people", kb_p, kb_p)
# media_compressed = associated_tasks(media_awards, data,"media",media,people)
...
# hosts, awards, final_nom, final_winners, final_pres = main_exec(OFFICIAL_AWARDS_1315,df,people,media)
